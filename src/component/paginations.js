...
import { BLOCK_SIZE, PAGE_SIZE } from './constants';
// css를 import 해서 코드 처럼 사용할 수 있도록 지정
...
function Paginations({data}) {
  ...
  // useEffect 에서 pagination 계산이 끝나지 않으면 출력하지 마라
  // pageno(x) → 1 → useEffect(서버로) → 출력 
  if(pagination==null)
    return;
